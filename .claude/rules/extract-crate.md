...
- `astro.rs` — Astro frontmatter extraction between `---` delimiters, plus `<script src="...">` references and inline `<script>` ESM imports from the template body (Astro bundles per-component client scripts at build time, so both shapes contribute to reachability).
